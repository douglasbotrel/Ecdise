...
export default function TarefasSemanaPage() {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-5 max-w-6xl mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Escolha o que você vai fazer nos próximos dias e marque conforme for concluindo.
          </p>
        </div>
        {podeGerenciarEquipe && (
          <button
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 bg-white rounded-xl border border-gray-100 p-3">
        {podeGerenciarEquipe ? (
          <select
            value={usuarioId}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4">
            <h2 className="text-sm font-semibold text-gray-900 mb-3">
              Minhas tarefas pendentes ({backlog.length})
            </h2>
            {backlog.length === 0 ? (
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4">
            <h2 className="text-sm font-semibold text-gray-900 mb-3">
              Planejado para esta semana ({planejadas.length})
            </h2>
            {planejadas.length === 0 ? (
              ...
              <div className="space-y-2">
                {planejadas.map(p => (
                  <div
                    key={p.id}
                    className={`flex items-start gap-2 p-2.5 rounded-lg border ${p.concluida ? 'border-green-100 bg-green-50/50' : 'border-gray-100'}`}
                  >
                    <button
